...
import nurture.settings
from django.conf import settings
from django.core.mail import send_mail
import razorpay


def plantsView(request, id):
    ...
    return render(request, 'plants.html', locals())
...
def cartDeleteView(request, cart_id):
    current_user = request.user
...
def add_to_wishlist(request, item_id):
    ...
    return redirect(f'/products/product-detail/{item_id}')
...
def wishlistView(request):
    ...
    witem_count = wishlist_items.count()
    context={
...
def search(request):
    ...
    products = Product.objects.filter(Q(description__icontains=keyword) | Q(name__icontains=keyword)).order_by('price')
...
def remove_from_wishlist(request,item_id):
    user = request.user
    wishItem=Wishlist.objects.get(id=item_id,user=user)
    wishItem.delete()
...
def checkout(request, address_id=None):
    ...
    if address_id:
        addresses = Address.objects.filter(id=address_id, customer=request.user)
    else:
...
def confirmation(request):
    ...
    return render (request,'products/confirmation.html',context)
...
def orderComplete(request,ordernumber) :
    ...
    return render(request,'products/order_completed.html',locals())
...
def cancelOrder(request, id):
    if request.method =='POST':
        ...
        client = razorpay.Client(auth=(nurture.settings.API_KEY, nurture.settings.RAZORPAY_SECRET_KEY))
